...
//Lambda function to return all categories from DynamoDB table
...
exports.getAllCategories = async () => {
    try {
        ...
        const categories = await Items.map((item) => ({
            categoryName: item?.categoryName?.S, //Extracting id
        ...
        return {
            statusCode: 500,
            body: JSON.stringify({error: 'Could not fetch categories'})
        };
    }
}
